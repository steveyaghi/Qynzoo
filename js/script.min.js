...
// then on transitionend rolls a new random target — so no two bubbles
// ever move in sync and the path never repeats.
(function initFloatingBubbles() {
    ...
    function driftOnce(bubble) {
        ...
        // hovered. Both sides go through the one property instead of one
        // writing an inline value the other can never take back.
    ...
    bubbles.forEach((bubble, i) => {
        ...
        setTimeout(() => {
            ...
            // only lasts while :hover matches. The instant it stops matching,
            // the cascade falls back to whatever this element's own inline
            // value is — without this, that would still be the last slow
            // drift duration, making the shrink-back just as slow as the
            // drift itself. Force it fast on the way out explicitly.
